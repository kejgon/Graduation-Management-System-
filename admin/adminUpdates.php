...
$query = "SELECT * FROM admin WHERE admin_number = '$adminNo'";
...
?>
            <div class="right" style="background-color:#FFF;">
                ...
                <div class="clearance-form">
                    <form method="post" action="adminUpdates.php" onsubmit="return formValidation()">
                        <div class="row">
                            <div class="col-10">
                                <label for="fname">Admin Full name:</label>
                                <input type="text" id="fullname" name="fullname"
                                    value="<?php echo $row['admin_fullname'];?>">

                            </div>
                            <div class="col-90">
                                <p>Admin No. : <?php echo $row['admin_number'];?></p>
                        ...
                        <div class="row">
                            <div class="col-10">
                                ...
                                <input type="password" id="currPassword" name="currPassword" value="">
...
if(isset($_POST['updates'])) {
    ...
    $currentPassword = trim(mysqli_real_escape_string($connection, $_POST['currPassword']));
    $newPassword = trim(mysqli_real_escape_string($connection, $_POST['newPassword']));
    ...
    if ($result) {
        ...
        $activity = "updated profile. (Admin No. " . $_SESSION['username'] . ")";
...
?>
    ...
    function formValidation() {
        ...
        var currentPasswordValu = currentPassword.value;
        if (currentPasswordValu.length == 0) {
            window.alert("Current password number is required"); // Display an alert if the current password is empty
            return false;
        }

        // Check if the password is at least 8 characters long
        if (currentPasswordValu.length < 8) {
            window.alert(
                "Current password must be at least 8 characters long"
            ); // Display an alert if the current password is less than 8 characters long
            return false;
        }
        ...
        for (var i = 0; i < currentPasswordValu.length; i++) {
            if (specialChars.indexOf(currentPasswordValu[i]) !== -1) {
                hasSpecialChar = true;
            }
            if (currentPasswordValu[i] >= 'A' && currentPasswordValu[i] <= 'Z') {
                hasUpperCase = true;
            }
            if (currentPasswordValu[i] >= 'a' && currentPasswordValu[i] <= 'z') {
                hasLowerCase = true;
            }
            if (currentPasswordValu[i] >= '0' && currentPasswordValu[i] <= '9') {
                hasNumber = true;
            }
        }
        ...
        if (!hasSpecialChar || !hasUpperCase || !hasLowerCase || !hasNumber) {
            window.alert(
                "Current password must at least contain uppercase, lowercase, digits, and special characters"
            ); // Display an alert if the current password does not meet the complexity requirements
            return false;
        }

        //! New PASSWORD VALIDATION
        var newPasswordValu = newPassword.value;
        if (newPasswordValu.length == 0) {
            window.alert("New password number is required"); // Display an alert if the new password is empty
            return false;
        }

        // Check if the password is at least 8 characters long
        if (newPasswordValu.length < 8) {
            window.alert(
                "New password must be at least 8 characters long"
            ); // Display an alert if the new password is less than 8 characters long
            return false;
        }
        // Check if the password contains at least one uppercase letter, one lowercase letter, one digit, and one special character
        var specialChars = "!#$%&'()*+,-./:;<=>?@[\]^_`{|}~";
        ...
        for (var i = 0; i < newPasswordValu.length; i++) {
            if (specialChars.indexOf(newPasswordValu[i]) !== -1) {
                hasSpecialChar = true;
            }
            if (newPasswordValu[i] >= 'A' && newPasswordValu[i] <= 'Z') {
                hasUpperCase = true;
            }
            if (newPasswordValu[i] >= 'a' && newPasswordValu[i] <= 'z') {
                hasLowerCase = true;
            }
            if (newPasswordValu[i] >= '0' && newPasswordValu[i] <= '9') {
                hasNumber = true;
            }
        }
        ...
        if (!hasSpecialChar || !hasUpperCase || !hasLowerCase || !hasNumber) {
            window.alert(
                "New password must at least contain uppercase, lowercase, digits, and special characters"
            ); // Display an alert if the new password does not meet the complexity requirements
            return false;
        }
