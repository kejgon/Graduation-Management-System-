...
if (isset($_GET['id'])) {
    ...
    $query = "DELETE FROM admin WHERE admin_number = $id";
    mysqli_query($connection, $query);

    
        // User activity tracker
        $role = $_SESSION['userRole'];
        $activity = "Deleted an Administrator. for (Admin No. " . $id. ")";
    ...
    echo '<script>';
    echo 'alert("An Admin has been deleted successfully!");';
    echo 'window.location.href = "viewadministrators.php";';
    echo '</script>';
} else {
    // If no Admin ID is provided, redirect to the viewStudents page
    redirect("viewadministrators.php");
}