...
<body>
    ...
    <div class="section">
        <div class="row">
            ...
            <?php 
// check if the 'id' and 'fulnames' parameters are present in the URL
if(isset($_GET['id']) && isset($_GET['fulnames'])) {
    // if so, store their values in the session variables
    $_SESSION['id'] = $_GET['id'];
    $_SESSION['fulnames'] = $_GET['fulnames'];
}
...
?>
            ...
            <div class="right" style="background-color:#FFF;">
                ...
                <h3> Update clearance status</h3>

                <div class="clearance-form">
                    <p style="text-align:center; color:green; background-color:white; border-radius:10px;">

                    <form id="libUpdateStatusForm" action="updateStatus.php" method="POST"
                        onsubmit="return validationFrom();">
...
if(isset($_POST['updateStatus'])){
    ...
    $stdFullName = $_SESSION['fulnames'];
    $status = $_POST['status'];


$sql = "SELECT * FROM hod_clearance_request WHERE std_regNo = $stdNo AND (stdStatus ='Pending' OR stdStatus ='Rejected')";
...
if (mysqli_num_rows($result) > 0) {
    echo "<p style='color:red'>Sorry! the student must be cleared by Head of the department(HOD) first! </p>";

} else {
    // Retrieving Student details
    $query = "SELECT * FROM library_clearance_request WHERE std_regNo = $stdNo";
    $result = mysqli_query($connection, $query);
    $row = mysqli_fetch_array($result);

    if ($row > 0) {
        $sql = "UPDATE library_clearance_request SET stdStatus = '$status' , emp_fullname = '$empFullName' WHERE std_regNo = $stdNo";
        if (mysqli_query($connection, $sql)) {

            
  ////! User activiy
$role = $_SESSION['position'];
$activity="Libarian Updating clearance status for Student No. ".$stdNo;
...
$insert = "INSERT INTO useractivity(username, fullname, role, type_of_activity, activity_time)
           VALUES('{$_SESSION['username']}', '$empFullName', '$role', '$activity', NOW())";
mysqli_query($connection, $insert);    


            echo "<p style='color:green'>The clearance status updated successfully</p>";
        } else {
