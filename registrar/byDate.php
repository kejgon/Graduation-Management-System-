    ...
    <div class="section">
        <div class="row">
            ...
            <div class="right" style="background-color:#FFF;">
                <h3>Clearance Reports by Date</h3>
                ...
                <div class="clearance-form">
                    ...
                    <table id="myTable" style="color:#000; text-align:left; padding-left:10px;">
                        ...
                        <tbody>
                            <?php
// Check if the 'Dates' key is set in the $_SESSION array
if (isset($_SESSION['dateFrom']) || isset($_SESSION['dateTo'])) {
    ...
    // Build the query
    $sql = "SELECT c.clr_id, c.std_fullname, c.std_regNo, c.faculty, c.department, c.levels, c.programs, c.specialization, c.campus, r.stdStatus, DATE(r.updated_at) AS updated_date 
            FROM clearance c 
            INNER JOIN fin_clearance_request r ON c.clr_id = r.clr_id";

    // Add the date search to the query if dateFrom is provided
    if (!empty($dateFrom)) {
        if (!empty($dateTo)) {
            $sql .= " WHERE DATE(r.updated_at) >= '$dateFrom' AND DATE(r.updated_at) <= '$dateTo'";
        } else {
            $sql .= " WHERE DATE(r.updated_at) = '$dateFrom'";
        }
    }
